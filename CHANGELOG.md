...
- **Relations popover** on the canvas toolbar: scope, connector visibility, labels, and a kind legend with per-kind visibility toggles and an add-by-name field. The legend draws each kind exactly as the canvas does — the plugin's first legend, which an open vocabulary makes necessary.
