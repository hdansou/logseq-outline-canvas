...
- **Relations popover** on the canvas toolbar: scope, connector visibility, labels, and a kind legend with per-kind visibility toggles, add-by-name, and `×` removal for kinds you added. Names added before their property exists are marked **not in graph** so a typo is not mistaken for a working kind. The legend draws each kind exactly as the canvas does — the plugin's first legend, which an open vocabulary makes necessary.
