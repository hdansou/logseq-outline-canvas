...
- **User-defined relationship kinds.** The vocabulary is no longer fixed: any property tagged `semantic-connector` (tag name configurable via `markerTag`) becomes a kind, as does any name listed in `customKinds` — for properties that aren't tagged or don't exist yet. Custom kinds take a color and dash pattern from a stable hash of their name, so adding one never reshuffles the others, and are directed unless listed in `undirectedKinds`.
- **Relations popover** on the canvas toolbar: scope, connector visibility, labels, and a kind legend with per-kind visibility toggles, add-by-name, and `×` removal for kinds you added. Names added before their property exists are marked **not in graph** so a typo is not mistaken for a working kind. An **Also in this graph** section lists node-typed properties that aren't kinds yet, so enabling one is a checkbox rather than retyping its name. The legend draws each kind exactly as the canvas does — the plugin's first legend, which an open vocabulary makes necessary.
- **`Cmd/Ctrl+Shift+G`** flips page ↔ graph scope, handled both by the host and inside the canvas iframe (host keybindings don't fire while the iframe holds focus — which is precisely when this shortcut is wanted).
